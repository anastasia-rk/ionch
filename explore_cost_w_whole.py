...
if __name__ == '__main__':
    ...
    *ps, g = thetas_true
                ...
                for iRun in range(nRuns):
                    ...
                    try:
                        ...
                        print(str(iRun + 1) + '-th run complete. Total evaluations: ' + str(
                            evals) + '. Total runtime: ' + str(toc_run - tic_run) + ' s.')
                    except:
                        # if the run breaks, we record its results as a bunch of NaNs and try to record it
                        print('Error encountered during optimisation.')
                        print(str(iRun + 1) + '-th run failed.')
                        runFailed[iRun] = True
                        Betas_BSPL = np.zeros_like(init_betas)
                        inner_cost = evals = outer_cost = gradient_cost = RMSE = np.nan
                        toc_run = tm.time()
                    ####################################################################################################################
    ...
    metadata = {'times': times, 'lambda': lambd, 'state_name': state_name, 'state_true': state_hidden_true,
                ...
                'knots': support, 'truth': theta_true, 'param_names': param_names, 'nruns': nRuns, 'param_names': param_names, 'log_scaled': inLogScale}
