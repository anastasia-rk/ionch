...
def optimise_first_segment(roi,input_roi,output_roi,support_roi,state_known_roi):
    ...
    class bsplineOutputSegment(pints.ForwardModel):
        ...
        def simulate(self, parameters, times):
            ...
            rhs = ode_both(times, fun_, Thetas_ODE)
...
if __name__ == '__main__':
    ...
    EK = -80
    thetas_true = [2.26e-4, 0.0699, 3.45e-5, 0.05462, 0.0873, 8.91e-3, 5.15e-3, 0.03158]
    ...
    solution = sp.integrate.solve_ivp(ode_both, [0,tlim[-1]], x0, args=[thetas_true], dense_output=True,method='LSODA',rtol=1e-8,atol=1e-8)
    x_ar = solution.sol(times)
    current_true = observation(times, x_ar, thetas_true)
    state_name = hidden_state_names= ['a','r']
    ################################################################################################################
    ...
    class bsplineOutputTest(pints.ForwardModel):
        ...
        def simulate(self, parameters, support, times):
            ...
            # the RHS must be put into an array
            dadr = ode_both(times, [fun_a, fun_r], Thetas_ODE)
    ...
    plt.plot(f_best,'-b',linewidth=1.5,label=r'Best cost:$H(\Theta_{true} \mid  \hat{C}, \bar{\mathbf{y}}) = $' + "{:.5e}".format(f_best[-1]))
    plt.legend(loc='best')
    plt.tight_layout()
